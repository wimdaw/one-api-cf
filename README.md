...
> **README 语言 / README Languages:** 🌐 [简体中文](README.md) | [English](README.en.md)
...
将本仓库 **fork 到自己 GitHub**,在 **Settings → Secrets and variables → Actions** 配置 3 个 secrets 后,推送到 `main` 分支即自动部署:

| Secret | 说明 |
|---|---|
| `CF_API_TOKEN` | Cloudflare API Token(需 Workers + D1 编辑权限) |
...
以 Pages 高级 `_worker.js` 模式部署,前端静态文件由 Pages 托管。Dashboard 创建 Pages 项目 → Settings 绑定 D1 + Analytics → 触发 **Deploy to Cloudflare Pages** workflow。详见 [`docs/DEPLOYMENT.md`](#方式三cloudflare-pages-部署高级-workerjs-模式)。

### 方式三:Docker 部署(自托管)

本地/自托管运行,支持 SQLite/MySQL/PostgreSQL,无需 Cloudflare:

```bash
docker compose up -d --build        # 默认 SQLite
docker compose --profile mysql up -d
docker compose --profile postgres up -d
```

详见 [`docs/DEPLOYMENT.md`](#方式四docker-部署自托管)。
...
- [部署指南(中文)](docs/DEPLOYMENT.md)
- [部署指南(English)](docs/DEPLOYMENT.en.md)
