...
export function Channels({ createMode = false, editRoute = false }: { createMode?: boolean; editRoute?: boolean }) {
  ...
  const fetchModelsMutation = useMutation({
    mutationFn: async ({ config, freeOnly }: { config: ChannelConfig; freeOnly?: boolean }) => {
      return apiClient.fetchChannelModels(config, freeOnly);
    },
    onSuccess: (response) => {
  ...
  const handleFetchModels = (freeOnly = false) => {
    ...
    if (!formData.type || !formData.endpoint) {
      addToast(t("channels.fillTypeEndpoint"), "error");
      return;
    }
    ...
    fetchModelsMutation.mutate({ config, freeOnly });
  };
