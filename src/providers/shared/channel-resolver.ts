...
export type ChannelResolution = {
    ...
    requestBody: any
    // multipart / form-data 请求的原始 body 与原 content-type,供直通透传
...
export const resolveChannel = async (
    ...
    let rawBody: ArrayBuffer | string | undefined;
    let upContentType: string | undefined;
    let isMultipartRequest = false;

    if (isMultipart || reqContentType.toLowerCase().includes("multipart/form-data")) {
        // multipart 请求(音频转录、图片编辑等文件上传):model 在 form 字段中,body 原样透传
        isMultipartRequest = true;
        try {
            const form = await c.req.formData();
            requestedModel = (form.get("model") as string) || "";
            // 保留原始 body 与 content-type 给上游
            rawBody = await c.req.arrayBuffer();
            upContentType = reqContentType;
        } catch (error) {
            return c.text("Invalid multipart form body", 400);
        }
